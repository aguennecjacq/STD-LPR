...
def P(input_array, output, r):
    ...
    # i follow the patch patern, j indexes the position we are currently at and k the depth.
    for patch_index in range(r ** 2):
        for pos_index in range(grid_x * grid_y):
            ...
            if pos_y + r > m:
                pos_y = m - r
            output[patch_index, pos_index] = input_array[pos_x + i, pos_y + j]
...
def P_inv(input_array, output, r, weights_map):
    ...
    for patch_index in range(r ** 2):
        for pos_index in range(grid_x * grid_y):
            ...
            if pos_y + r > m:
                pos_y = m - r
            output[pos_x + i, pos_y + j] += input_array[patch_index, pos_index] * weights_map[pos_x + i, pos_y + j]
